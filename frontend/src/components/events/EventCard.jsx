...
import { FileText, Sheet, Image, ArrowRight } from 'lucide-react';
...
const CAT_EMOJI = {
  Technology: '💻', Science: '🔬', Arts: '🎨', Sports: '🏅', Workshop: '🛠️',
};
...
export default function EventCard({ event }) {
  ...
  return (
    <Link to={`/events/${event.id}`}>
      <div className="card overflow-hidden hover:shadow-card-hover transition-all duration-200 hover:-translate-y-0.5 group cursor-pointer h-full flex flex-col">
        {/* Color bar */}
        <div className="h-1.5 w-full" style={{ background: color }} />

        <div className="p-4 flex flex-col flex-1">
          {/* Category badge */}
          <span className={`badge mb-3 w-fit ${style}`}>
          ...
          <div className="flex items-center gap-1.5 pt-3 border-t border-gray-100">
            ...
            <span
              className="ml-auto w-7 h-7 rounded-full flex items-center justify-center text-white text-sm group-hover:rotate-45 transition-transform duration-200"
              style={{ background: color }}
            >
              <ArrowRight size={13} />
            </span>
