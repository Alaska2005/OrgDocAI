...
import { CalendarDays, FileText, Image, Users, TrendingUp, ArrowRight } from 'lucide-react';
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
        ...
        {STAT_CARDS.map(({ key, label, icon: Icon, color, link }, i) => (
          ...
          >
            <Link to={link || '#'} className="block">
              <div className="card p-5 hover:shadow-card-hover transition-shadow cursor-pointer group">
                ...
                <p className="text-xs text-emerald-500 mt-1 flex items-center gap-1">
                  <TrendingUp size={11} /> Updated today
                </p>
              </div>
            </Link>
          </motion.div>
      ...
      {/* Activity Timeline */}
      <div>
        <h3 className="font-heading font-bold text-base text-gray-900 mb-4">Recent Activity</h3>
            ...
            <div key={log.id} className="flex gap-3 items-start">
              ...
              <div className="flex-1 pt-0.5">
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {format(new Date(log.createdAt), 'MMM d, yyyy · h:mm a')} · by {log.user?.name}
                </p>
              </div>
            </div>
          ))}
